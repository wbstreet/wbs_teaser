...
function get_teaser_type_parent_page($section_id) {
    global $database;
    $sql = 'SELECT * FROM `'.TABLE_PREFIX.'mod_teasers_type_parent` WHERE `section_id`='.(int)$section_id;
...
function get_teaser_type_dir($section_id) {
    global $database;
    $sql = 'SELECT * FROM `'.TABLE_PREFIX.'mod_teasers_type_dir` WHERE `section_id`='.(int)$section_id;
    $r = $database->query($sql);
    if ($database->is_error()) echo $database->get_error();
    $r = $r->fetchRow();
    return $r;
...
function get_teaser_type_any_urls($section_id) {
    global $database;
    $sql = 'SELECT * FROM `'.TABLE_PREFIX.'mod_teasers_type_any_urls` WHERE `section_id`='.(int)$section_id;
    $r = $database->query($sql);
    if ($database->is_error()) echo $database->get_error();
    return $r;
}
